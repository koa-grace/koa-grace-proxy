...
function requestWap(param, options, callback) {
   ...
   * @param  {String}  businessCode 业务code
   * @return {String}               输出到error.log的信息
   */
  function errLogInfo(startTime, duration, status, businessCode) {
    // 页面url
    ...
    // method
    let method = param.ctx.method;
    // 业务code
    businessCode = businessCode || '-';

    return `| ${startTime} | ${ip} | ${status} | ${businessCode} | ${method} | ${pageUrl} | ${interfaceUrl} | ${ua} | ${duration} |`
  }
  ...
  function _createReq() {
    ...
    return Request(opt, (err, httpResponse, data) => {
      ...
      if (!err && data) {
        ...
        let path = httpResponse && httpResponse.request && httpResponse.request.path || '';

        // 接口不存在，一般情况4xx、5xx，某些情况下会302到错误页面，错误页面的path以'/message?'开头
        if (~path.indexOf('/message?') || +status >= 400 && +status < 600) {
          let err = new Error('Request does not exist!');
          error(`proxy error : ${errLogInfo(startTime, duration, status)}`, err);  
        } else {
          // 输出所有proxy信息到服务access日志里
          console.log(`proxy access : ${errLogInfo(startTime, duration, status, data && data.code || '-')}`);
        }
