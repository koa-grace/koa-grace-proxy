...
function requestWap(param, options, callback) {
  ...
  // 创建请求
  function _createReq() {
    return Request(opt, function(err, httpResponse, data) {
      if (!err && data) {
        debug('proxy success : ' + opt.uri);
      } else {
        err = err || new Error('THE REQUEST NO RESPONSE!');
        err.http_code = httpResponse && httpResponse.statusCode || 'NULL';

        error('proxy error : ' + opt.uri, err);

        // 请求失败重试
        if(retry && retryNum) {
          debug('PROXY RETRY: ' + opt.uri + ' FAILED, RETRY ' + retryNum + ' TIME');
          retryNum--;
          return _createReq()
        } else {
          data = {};
